...
module Regexp::SyntaxFzs
  ...
  class InvalidVersionNameError < Regexp::SyntaxFzs::SyntaxError
    def initialize(name)
      super "Invalid version name '#{name}'. Expected format is '#{VERSION_FORMAT}'"
    end
  end

  class UnknownSyntaxNameError < Regexp::SyntaxFzs::SyntaxError
    def initialize(name)
  ...
  def new(name)
    warn 'Regexp::SyntaxFzs.new is deprecated in favor of Regexp::SyntaxFzs.for. '\
  ...
  def version_class(version)
    return Regexp::SyntaxFzs::Any if ['*', 'any'].include?(version.to_s)
