...
class Regexp::LexerFzs
  ...
  def lex(input, syntax = nil, options: nil, collect_tokens: true, &block)
    syntax = syntax ? Regexp::SyntaxFzs.for(syntax) : Regexp::SyntaxFzs::CURRENT
    ...
    Regexp::ScannerFzs.scan(input, options: options, collect_tokens: false) do |type, token, text, ts, te|
      ...
      current = Regexp::TokenFzs.new(type, token, text, ts + shift, te + shift,
  ...
  def break_literal(token)
    ...
    token_1 = Regexp::TokenFzs.new(:literal, :literal, lead,
              token.ts, (token.te - last.length),
              nesting, set_nesting, conditional_nesting)
    token_2 = Regexp::TokenFzs.new(:literal, :literal, last,
  ...
  def break_codepoint_list(token)
    ...
    token_1 = Regexp::TokenFzs.new(:escape, :codepoint_list, lead + '}',
              token.ts, (token.te - tail.length),
              nesting, set_nesting, conditional_nesting)
    token_2 = Regexp::TokenFzs.new(:escape, :codepoint_list, '\u{' + tail,
  ...
  def merge_condition(current, last)
    token = Regexp::TokenFzs.new(:conditional, :condition, last.text + current.text,
