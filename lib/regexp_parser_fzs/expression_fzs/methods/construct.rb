...
module Regexp::ExpressionFzs
  module Shared
    module ClassMethods
      # Convenience method to init a valid Expression without a Regexp::TokenFzs
      def construct(params = {})
        attrs = construct_defaults.merge(params)
        options = attrs.delete(:options)
        token_args = Regexp::TOKEN_KEYS_FZS.map { |k| attrs.delete(k) }
        token = Regexp::TokenFzs.new(*token_args)
      ...
      def token_class
        ...
        # TODO: synch exp class, token class & type names for this in v3.0.0
          ...
          Regexp::SyntaxFzs::Token::Meta
        else
          Regexp::SyntaxFzs::Token.const_get(name.split('::')[2])
        end
      end
    end
