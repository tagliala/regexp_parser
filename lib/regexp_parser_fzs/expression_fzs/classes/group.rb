...
module Regexp::ExpressionFzs
  module Group
    class Base < Regexp::ExpressionFzs::Subexpression
    end
    ...
    class Options < Group::Base
      ...
      def quantify(*args)
        if token == :options_switch
          raise Regexp::ParserFzs::Error, 'Can not quantify an option switch'
        else
          super
        end
      end
  ...
  module Assertion
    class Base < Regexp::ExpressionFzs::Group::Base; end
