...
module Regexp::ExpressionFzs
  ...
  class Quantifier
    include Regexp::ExpressionFzs::Shared
    ...
    def deprecated_old_init(token, text, _min, _max, _mode = :greedy)
      warn "Calling `Expression::Base#quantify` or `#{self.class}.new` with 4+ arguments "\
           ...
           "Please pass a Regexp::TokenFzs instead, e.g. replace `token, text, min, max, mode` "\
           "with `::Regexp::TokenFzs.new(:quantifier, token, text)`. min, max, and mode "\
